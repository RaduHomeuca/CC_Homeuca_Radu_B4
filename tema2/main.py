...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/customers':
            ...
            for names in root.iter('CustomerName'):
                for customer in root.findall('Customer'):
                    # find the CustomerName child element
                    name_elem = customer.find('CustomerName')
                    if name_elem is not None and name_elem.text == names.text:
                        data += customer.get('id')+'.'+names.text + ','
        ...
        elif path_parts[1] == 'customercif' and len(path_parts) == 3:
            customer_id = path_parts[2]
            print(customer_id)
            print(path_parts)
            self.send_response(200)
            ...
            self.wfile.write(bytes(json.dumps(message), "utf8"))
        elif path_parts[1] == 'customercnp' and len(path_parts) == 3:
            customer_id = path_parts[2]
            print(customer_id)
            print(path_parts)
            self.send_response(200)
